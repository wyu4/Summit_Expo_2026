...
function SponsorshipPDF({ pdfUrl = "/sponsorship-package.pdf" }: { pdfUrl?: string }) {
  ...
  useEffect(() => {
    ...
    // Backdrop
    gsap.fromTo(bd, { opacity: 0 }, { opacity: 1, duration: 0.4, ease: "power2.out" });

    // Viewer modal
    gsap.fromTo(
      vw,
      { opacity: 0, scale: 0.80, rotateX: -18, rotateY: 5, y: 70, filter: "blur(10px)" },
    ...
    gsap.fromTo(".spdf-scan",
      ...
      { scaleX: 1, opacity: 1, duration: 0.75, ease: "power3.inOut",
        transformOrigin: "left center", delay: 0.18 }
    );

    // Corners snap in
    gsap.fromTo(".spdf-corner",
      { scale: 0, opacity: 0 },
      { scale: 1, opacity: 1, stagger: 0.07, duration: 0.30,
        ease: "back.out(3)", delay: 0.28 }
    );
    ...
    [o1Ref, o2Ref, o3Ref].forEach((r, i) => {
      ...
      gsap.fromTo(r.current,
        { scale: 0.2, opacity: 0 },
        { scale: 1, opacity: 1, duration: 0.65, ease: "back.out(1.4)", delay: 0.22 + i * 0.14 }
      );
    });

    // HUD bar
    gsap.fromTo(".spdf-hud-top",
      { opacity: 0, y: -14 },
      { opacity: 1, y: 0, duration: 0.45, ease: "power3.out", delay: 0.32 }
    );
    gsap.fromTo(".spdf-hud-bottom",
      { opacity: 0, y: 14 },
      { opacity: 1, y: 0, duration: 0.45, ease: "power3.out", delay: 0.38 }
    );

    // Signal bars
    gsap.fromTo(".spdf-sigbar",
      { scaleY: 0 },
      { scaleY: 1, stagger: 0.04, duration: 0.22, ease: "power2.out",
        transformOrigin: "bottom center", delay: 0.5 }
    );

    // Embed fades last
    gsap.fromTo(".spdf-embed-wrap",
      { opacity: 0 },
      { opacity: 1, duration: 0.55, ease: "power2.out", delay: 0.50 }
    );
  ...
  const closeIt = () => {
    ...
    gsap.timeline({
      ...
      .to(vw, {
        opacity: 0, scale: 0.86, rotateX: -10, y: 28, filter: "blur(6px)",
            ...
            <div className="spdf-hud-top">
              <div className="spdf-hud-top__left">
                <div className="spdf-bar__dots">
                  <span className="spdf-dot spdf-dot--close" onClick={closeIt} title="Close" role="button" aria-label="Close modal" />
              ...
              <div className="spdf-hud__net">
                <span className="spdf-hud__net-label"><i className="fa-solid fa-wifi" /></span>
                <div className="spdf-hud__net-bar">
                  <div className="spdf-hud__net-fill" style={{ width: `${sigPct}%` }} />
                </div>
              </div>
        ...
        <div className="spdf-card__holo" aria-hidden="true">
          ...
          <div className="spdf-page spdf-page--1">
            ...
            {Array.from({ length: 5 }, (_, i) => (
              <div key={i} className="spdf-line" style={{
      ...
      <div className="pi-countdown__units">
        {units.map(({ val, label }, i) => (
          <div key={label} style={{ display: "contents" }}>
            {i > 0 && (
              <span className="pi-countdown__sep" aria-hidden="true">:</span>
            )}
            ...
            <div className="pi-terminal__body">
              {[
                ...
                <div key={key} className="pi-terminal__line">
                  ...
                  <span className={`pi-terminal__val${accent ? " pi-terminal__val--accent" : ""}`}>
                    {val}
                  </span>
                </div>
              ))}
          ...
          <div className="pi-card pi-animate">
            ...
            <div className="pi-card__accent-stars" aria-hidden="true">
              {Array.from({ length: 6 }, (_, i) => (
                <span key={i} className="pi-star"
                  style={{ animationDelay: `${i * 0.38}s`, animationDuration: `${2 + i * 0.3}s` }}>
                  ✦
                </span>
              ))}
            </div>
