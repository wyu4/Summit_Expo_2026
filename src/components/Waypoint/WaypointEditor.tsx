/**
 * WaypointEditor — dev-only rocket path planner.
 * Stores ABSOLUTE pixel coordinates + the screen resolution they were
 * recorded on. RocketPath scales them to any other screen size.
 *
 * Press W or click "✦ PATH" (bottom-right) to open.
 */
...
export function WaypointEditor() {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Escape') { setAddMode(false); setOpen(false); }
    };
  ...
  const onPanelUp = () => { panelDrag.current = null; };

  // ── Node drag ─────────────────────────────────────────────────
  const onNodeDown = (e: React.PointerEvent<SVGCircleElement>, idx: number, kind: DragKind) => {
    e.stopPropagation();
    e.currentTarget.setPointerCapture(e.pointerId);
  ...
  useEffect(() => {
    const onMove = (e: PointerEvent) => {
      ...
      const nx = Math.max(0, Math.min(window.innerWidth,  ox + (e.clientX - sx)));
    ...
    const onUp = () => { nodeDrag.current = null; };
  ...
  // ── Sidebar actions ────────────────────────────────────────────
  const addHandle = (idx: number) => setPoints(prev => prev.map((p, i) =>
    i === idx ? { ...p, cpx: p.x + 40, cpy: Math.max(0, p.y - 40) } : p));
  const removeHandle = (idx: number) => setPoints(prev => prev.map((p, i) => {
    if (i !== idx) return p;
    const { cpx: _, cpy: __, ...rest } = p; return rest;
  }));
  const moveUp = (idx: number) => {
    if (idx === 0) return;
    setPoints(prev => { const a = [...prev]; [a[idx-1],a[idx]]=[a[idx],a[idx-1]]; return a; });
    setSel(idx - 1);
  };
  ...
  const del = (idx: number) => { setPoints(prev => prev.filter((_,i)=>i!==idx)); setSel(null); };
  const editField = (idx: number, field: keyof Waypoint, val: string) => {
    const n = parseFloat(val); if (isNaN(n)) return;
    setPoints(prev => prev.map((p,i) => i===idx ? {...p,[field]:Math.round(n)} : p));
  };
  const copyJSON = () => {
...
${ptLines}
  ...
  const clearAll = () => { setPoints(() => []); setSel(null); setAddMode(false); };
    ...
    <>
      ...
      >
        ...
        <g transform={`translate(0,${-scrollY})`} style={{ pointerEvents: 'none' }}>
          ...
          {points.map((p, i) => p.cpx != null ? (
            <line key={`hl${i}`}
              x1={p.x} y1={p.y} x2={p.cpx} y2={p.cpy!}
        ...
        {points.map((p, i) => {
          ...
          return (
            <g key={`n${i}`} style={{ pointerEvents: 'all' }}>
              {sel === i && <circle cx={p.x} cy={cy} r={17} className="wpe-sel-ring" />}
              <circle cx={p.x} cy={cy} r={11}
                className={`wp-node wpe-node-hit${sel===i?' wp-node--sel':''}`}
                onPointerDown={e => onNodeDown(e, i, 'point')} />
              <text x={p.x} y={cy+4} className="wpe-node-label" style={{pointerEvents:'none'}}>{i}</text>
            </g>
          );
        })}
        ...
        {points.map((p, i) => {
          ...
          if (cy < -60 || cy > window.innerHeight + 60) return null;
          return (
            <circle key={`h${i}`} cx={p.cpx} cy={cy} r={7}
      ...
      >
        ...
        <div className="wpe-actions" style={{ paddingTop: 0 }}>
          ...
          <button className="wpe-btn" onClick={() => setSel(null)}>Deselect</button>
        </div>
            ...
            >
              <div className="wpe-item-top">
                ...
                <div className="wpe-item-coords">
                  <label>x<input type="number" value={p.x} step={1}
                    onChange={e=>editField(i,'x',e.target.value)}
                    onClick={e=>e.stopPropagation()} /></label>
                  <label>y<input type="number" value={p.y} step={1}
                ...
                <div className="wpe-item-btns">
                  ...
                  <button className="wpe-del" onClick={e=>{e.stopPropagation();del(i)}}>✕</button>
                </div>
              </div>
              {p.cpx != null ? (
                <div className="wpe-handle-row">
                  <span className="wpe-handle-label">↳ handle</span>
                  <label>hx<input type="number" value={p.cpx} step={1}
                    onChange={e=>editField(i,'cpx',e.target.value)}
                    onClick={e=>e.stopPropagation()} /></label>
                  <label>hy<input type="number" value={p.cpy} step={1}
                    onChange={e=>editField(i,'cpy',e.target.value)}
                    onClick={e=>e.stopPropagation()} /></label>
                  <button className="wpe-rm-handle" onClick={e=>{e.stopPropagation();removeHandle(i)}}>✕</button>
                </div>
              ) : (
                <button className="wpe-add-handle" onClick={e=>{e.stopPropagation();addHandle(i)}}>
                  ＋ curve handle
                </button>
              )}
            </div>
