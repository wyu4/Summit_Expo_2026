...
export function GalleryBridge() {
  ...
  useEffect(() => {
    ...
    interface S { x:number; y:number; r:number; op:number; ph:number; sp:number; hue:number; }
    let stars: S[] = [], raf = 0, t = 0, running = false;
    ...
    const loop = () => {
      ...
      for (const s of stars) {
        ...
        ctx.fillStyle = `hsla(${s.hue},65%,70%,${al * 0.06})`; ctx.fill();
    ...
    obs.observe(canvas);
    return () => { stop(); obs.disconnect(); ro.disconnect(); };
  }, []);
      ...
      ScrollTrigger.create({
        ...
        onEnter() {
          ...
          gsap.fromTo('.gb-scan',
            { scaleX: 0 },
            {
              scaleX: 1, stagger: 0.12, duration: 0.5,
              ease: 'power3.inOut', transformOrigin: 'left center', delay: 0.35,
              onComplete() {
                gsap.to('.gb-scan', { opacity: 0, duration: 0.4, stagger: 0.08 });
              },
            },
          );
          // Corner brackets pop in
          gsap.fromTo('.gb-corner',
            { opacity: 0, scale: 0 },
            { opacity: 1, scale: 1, stagger: 0.03, duration: 0.25, ease: 'back.out(2.5)', delay: 0.4 },
          );
          // Captions fade up
          gsap.fromTo('.gb-caption',
            { opacity: 0, y: 10 },
            { opacity: 1, y: 0, stagger: 0.12, duration: 0.45, ease: 'power2.out', delay: 0.65 },
          );
          // Designation tags
          gsap.fromTo('.gb-desig',
            { opacity: 0 },
  ...
  return (
    <section ref={sectionRef} className="gb" id="gallery">
      ...
      <div className="gb__bridge-line gb__bridge-line--bottom" aria-hidden="true" />
    </section>
  );
}