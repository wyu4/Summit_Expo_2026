/**
 * Register — "Initiate Launch Sequence"
 *
 * Two registration paths side by side:
 *   - Exhibitor (submit a project)
 *   - Attendee  (sign up to attend)
 *
 * Each card links to a Google Form.
 * Below: a "What happens next" timeline so registrants know what to expect.
 *
 * Placement in App.tsx: after FAQ, before the page footer / ATTEND CTA.
 * i.e. <FAQ /> → <Register /> → (ATTEND / footer)
 */
...
//  Replace these with your real Google Form URLs 
const EXHIBITOR_FORM = 'https://forms.google.com/YOUR_EXHIBITOR_FORM_ID';
const ATTENDEE_FORM  = 'https://forms.google.com/YOUR_ATTENDEE_FORM_ID';
    ...
    const resize = () => {
      ...
      stars = Array.from({ length: 160 }, () => {
        ...
        return { x:Math.random()*canvas.width, y:Math.random()*canvas.height,
          ...
          op:Math.random()*0.65+0.2, ph:Math.random()*Math.PI*2,
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ScrollTrigger.create({
        ...
        onEnter() {
          gsap.fromTo('.reg-eyebrow',
            { opacity:0, y:20 }, { opacity:1, y:0, duration:0.55, ease:'power3.out' }
          );
          gsap.fromTo('.reg-title',
            { opacity:0, y:32, filter:'blur(6px)' },
            { opacity:1, y:0, filter:'blur(0px)', duration:0.75, ease:'power3.out', delay:0.1 }
          );
          gsap.fromTo('.reg-sub',
            { opacity:0, y:16 }, { opacity:1, y:0, duration:0.55, ease:'power2.out', delay:0.25 }
          );
          gsap.fromTo('.reg-card',
            { opacity:0, y:40, scale:0.94 },
            { opacity:1, y:0, scale:1, stagger:0.14, duration:0.70, ease:'power3.out', delay:0.35 }
          );
        },
      });

      ScrollTrigger.create({
        trigger: '.reg-timeline', start: 'top 82%',
        onEnter() {
          gsap.fromTo('.reg-timeline__title',
            { opacity:0, y:20 }, { opacity:1, y:0, duration:0.5, ease:'power3.out' }
          );
          gsap.fromTo('.reg-step',
            { opacity:0, x:-24 },
            { opacity:1, x:0, stagger:0.12, duration:0.55, ease:'power3.out', delay:0.15 }
          );
          // Draw the connecting line
          gsap.fromTo('.reg-timeline__track',
            { scaleY:0 }, { scaleY:1, duration:0.8, ease:'power2.inOut', transformOrigin:'top center', delay:0.1 }
          );
        },
      });
    }, sectionRef);
    ...
    <section ref={sectionRef} id="register" className="reg">
      ...
      <div className="reg__inner">
        ...
        <div className="reg__header">
          ...
          <h2 className="reg-title" style={{ opacity: 0 }}>Register</h2>
        ...
        <div className="reg-cards">
          ...
          <div className="reg-card reg-card--exhibitor" style={{ opacity: 0 }}>
            ...
            <ul className="reg-card__features">
              <li><i className="fa-solid fa-check" /> Open to all schools, grades 9–12</li>
              <li><i className="fa-solid fa-check" /> Solo or team (up to 4)</li>
              <li><i className="fa-solid fa-check" /> Any STEM field — finished or in-progress</li>
              <li><i className="fa-solid fa-check" /> Physical resources can be requested</li>
              <li><i className="fa-solid fa-check" /> Judged by industry professionals</li>
            </ul>
            <a
              href={EXHIBITOR_FORM}
              target="_blank"
            ...
            <p className="reg-card__note">
              <i className="fa-solid fa-circle-info" /> Applications reviewed by the team — you'll hear back by email.
            </p>
          </div>
          ...
          <div className="reg-card reg-card--attendee" style={{ opacity: 0 }}>
            ...
            <ul className="reg-card__features">
              <li><i className="fa-solid fa-check" /> Free admission, open to all ages</li>
              <li><i className="fa-solid fa-check" /> Watch live pitches in Act I</li>
              <li><i className="fa-solid fa-check" /> Explore booths & demos in Act II</li>
              <li><i className="fa-solid fa-check" /> Score exhibits like a judge</li>
              <li><i className="fa-solid fa-check" /> Live awards ceremony</li>
            </ul>
            <a
              href={ATTENDEE_FORM}
              target="_blank"
            ...
            <p className="reg-card__note">
              <i className="fa-solid fa-circle-info" /> No approval needed — sign up and show up!
            </p>
          </div>
          ...
          <div className="reg-timeline__body">
            ...
            {TIMELINE.map((item, i) => (
              <div key={i} className="reg-step" style={{ opacity: 0 }}>
                <div className="reg-step__node" style={{ '--step-color': item.color } as React.CSSProperties}>
                  <i className={`fa-solid ${item.icon}`} />
                  <div className="reg-step__pulse" />
                </div>
                <div className="reg-step__content">
                  <span className="reg-step__num" style={{ color: item.color }}>{item.step}</span>
        ...
        <div className="reg-footer">
          <p className="reg-footer__text">
            Questions? <a href="mailto:XXXXXX" className="reg-footer__link">Shoot us an email</a> — we're happy to help.
          </p>
        </div>
