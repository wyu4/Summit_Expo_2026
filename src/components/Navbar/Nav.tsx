...
function StarDot({
  mag, active, hovered, large = false,
}: {
  mag: number; active: boolean; hovered: boolean; large?: boolean;
}) {
  ...
  return (
    <svg
      ...
      width={sz} height={sz} viewBox={`0 0 ${sz} ${sz}`} overflow="visible"
    >
      ...
      <circle cx={cx} cy={cx} r={r}     fill={active ? "#CE3072" : "white"}
        className="star-core" />
      {lit && (
        <>
          <line x1={cx} y1={0}  x2={cx} y2={sz} stroke={active ? "#CE3072" : "white"} strokeWidth={0.7} opacity={0.45} className="star-spike" />
          <line x1={0}  y1={cx} x2={sz} y2={cx} stroke={active ? "#CE3072" : "white"} strokeWidth={0.7} opacity={0.45} className="star-spike" />
          <line x1={sz*0.16} y1={sz*0.16} x2={sz*0.84} y2={sz*0.84} stroke={active ? "#CE3072" : "white"} strokeWidth={0.4} opacity={0.22} />
          <line x1={sz*0.84} y1={sz*0.16} x2={sz*0.16} y2={sz*0.84} stroke={active ? "#CE3072" : "white"} strokeWidth={0.4} opacity={0.22} />
        </>
      )}
    </svg>
  );
...
export function Nav() {
  ...
  const [starPositions, setStarPositions] = useState<[number, number][]>(generatePositions);
  ...
  useEffect(() => { starPositionsRef.current = starPositions; }, [starPositions]);
  ...
  useEffect(() => {
    ...
    const st = ScrollTrigger.create({
      ...
      onEnter() {
        gsap.to(nav, { xPercent: -50, yPercent: 0, opacity: 1, duration: 0.75, ease: "power3.out" });
        gsap.to(drawProgress, { current: 1, duration: 1.6, delay: 0.5, ease: "power2.inOut" });
        gsap.fromTo(".nav__item",
          { scale: 0, opacity: 0 },
          { scale: 1, opacity: 1, stagger: 0.1, duration: 0.55, ease: "back.out(2.8)", delay: 0.35 }
        );
      },
      onLeaveBack() {
        gsap.to(nav, { xPercent: -50, yPercent: -130, opacity: 0, duration: 0.4, ease: "power2.in" });
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      requestAnimationFrame(() => {
        ...
        let best = LINKS[0].id, bestD = Infinity;
        LINKS.forEach(({ id }) => {
...
useEffect(() => {
  ...
  window.addEventListener('scroll', () => { centresDirty = true; }, { passive: true });
  ...
  const draw = () => {
    ...
    for (let i = 0; i < total; i++) {
      ...
      const ex = a.x + (b.x - a.x) * sp, ey = a.y + (b.y - a.y) * sp;
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      MOBILE_EDGES.forEach(([ai, bi], ei) => {
        ...
        // Glow
        ctx.save(); ctx.globalAlpha=0.18*prog; ctx.strokeStyle="#CE3072";
        ctx.lineWidth=6; ctx.filter="blur(6px)";
        ctx.beginPath(); ctx.moveTo(a.x,a.y); ctx.lineTo(ex,ey); ctx.stroke(); ctx.restore();
        // Mid
        ctx.save(); ctx.globalAlpha=0.35*prog; ctx.strokeStyle="#8ab4d4";
        ctx.lineWidth=2; ctx.filter="blur(2px)";
        ctx.beginPath(); ctx.moveTo(a.x,a.y); ctx.lineTo(ex,ey); ctx.stroke(); ctx.restore();
        // Dashed
        ctx.save(); ctx.globalAlpha=0.65*prog; ctx.strokeStyle="rgba(180,220,255,0.85)";
        ctx.lineWidth=1.2; ctx.setLineDash([6,8]); ctx.lineDashOffset=-Date.now()*0.008; ctx.lineCap="round";
        ctx.beginPath(); ctx.moveTo(a.x,a.y); ctx.lineTo(ex,ey); ctx.stroke(); ctx.restore();
        // Solid
        ctx.save(); ctx.globalAlpha=0.5*prog; ctx.strokeStyle="rgba(220,240,255,0.9)";
        ctx.lineWidth=0.8; ctx.setLineDash([]);
        ctx.beginPath(); ctx.moveTo(a.x,a.y); ctx.lineTo(ex,ey); ctx.stroke(); ctx.restore();
      });
    };
    overlayRafRef.current = requestAnimationFrame(draw);
    return () => { cancelAnimationFrame(overlayRafRef.current); window.removeEventListener("resize", resize); };
  }, []);
  ...
  const openMenu = useCallback(() => {
    ...
    mobileStarRefs.current.forEach((el, i) => {
      ...
      gsap.fromTo(el,
        { scale: 0, opacity: 0 },
        { scale: 1, opacity: 1, duration: 0.55, delay: 0.08 + i * 0.1, ease: "back.out(2.5)" }
      );
    });

    // Edges draw sequentially (spine first, then braces)
    MOBILE_EDGES.forEach((_, ei) => {
      gsap.to(lineProgress.current, { [ei]: 1, duration: 0.55, delay: 0.4 + ei * 0.1, ease: "power2.inOut" });
    });

    // Burger → X
    gsap.to(".burger-bar-1", { rotation: 45,  y:  7, duration: 0.3, ease: "power2.inOut" });
    gsap.to(".burger-bar-2", { opacity: 0, x: 8, duration: 0.2 });
    gsap.to(".burger-bar-3", { rotation: -45, y: -7, duration: 0.3, ease: "power2.inOut" });
  }, []);
  ...
  const closeMenu = useCallback(() => {
    ...
    mobileStarRefs.current.forEach((el, i) => {
      if (!el) return;
      gsap.to(el, { scale: 0, opacity: 0, duration: 0.3, delay: i * 0.04, ease: "power2.in" });
    });
    MOBILE_EDGES.forEach((_, ei) => {
      gsap.to(lineProgress.current, { [ei]: 0, duration: 0.25, ease: "power2.in" });
    });
    gsap.to(overlay, {
      opacity: 0, duration: 0.35, delay: 0.2, ease: "power2.in",
      onComplete: () => { menuOpenRef.current = false; setMenuOpen(false); },
    });

    // X → Burger
    gsap.to(".burger-bar-1", { rotation: 0, y: 0, duration: 0.3, ease: "power2.inOut" });
    gsap.to(".burger-bar-2", { opacity: 1, x: 0, duration: 0.2, delay: 0.1 });
    gsap.to(".burger-bar-3", { rotation: 0, y: 0, duration: 0.3, ease: "power2.inOut" });
  }, []);

  const toggleMenu = useCallback(() => {
    if (menuOpenRef.current) closeMenu(); else openMenu();
  }, [openMenu, closeMenu]);
      ...
      >
        ...
        <button className="nav__logo" onClick={() => handleClick("hero")}>
          <span className="nav__logo-star" aria-hidden="true">
            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" className="nav__logo-svg">
              <circle cx="10" cy="10" r="8.5" fill="#CE3072" opacity="0.10" className="logo-halo" />
          ...
          <span className="nav__logo-text">SUMMIT<em>EXPO</em></span>
        </button>
              ...
              >
                ...
                {isActive && (
                  <>
                    ...
                    <span className="nav__item-orbit nav__item-orbit--2" aria-hidden="true" />
                  </>
                )}
              </button>
            );
        ...
        <button className="nav__cta nav__desktop-only" onClick={() => handleClick("register")}>
      ...
      >
        ...
        {LINKS.map(({ id, label, mag, greekLetter }, i) => {
          ...
          return (
            <button
              key={id}
              ref={(el) => { mobileStarRefs.current[i] = el; }}
            ...
            >
              ...
              <span className="overlay-star__dist">{(mag * 12.4).toFixed(1)} ly</span>
              {isActive && (
                <>
        ...
        >
          <span className="nav__overlay-close__x" aria-hidden="true">✕</span>
